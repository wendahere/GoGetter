...
def findSheet(UID):
    ...
    for sheet in worksheet_list:
        # Ommit the "Logs" file
        if ("Log" not in sheet.title and "log" not in sheet.title and "DO NOT DELETE" not in sheet.title):
...
def tap(gettext):
    ...
    if (type(gettext) == int):
        ...
        if (sheet == UID_NOT_FOUND):
            ...
            row = findRow(sheetsData, gettext)
            ...
            if (remainingPoints == INSUFFICIENT_POINTS):
                ...
                updateLogs(gettext, details, remainingPoints, pointstodeduct)
